...
```bash
mkdir -p .ralph-loop && cat > .ralph-loop/ralph-loop.local.md << 'EOF'
---
active: true
iteration: 0
maxIterations: 100
...
```bash
grep '^iteration:' .ralph-loop/ralph-loop.local.md
```

## State File Format

The state file at `.ralph-loop/ralph-loop.local.md` uses YAML frontmatter:
...
Add `.ralph-loop/ralph-loop.local.md` to your `.gitignore`.
