...
# OpenRalph PRD Converter

Converts existing PRDs to the prd.json format that OpenRalph uses for autonomous execution.

---

## The Job

Take `.ralph/prd.md` and convert it to `.ralph/prd.json`.
...
**Each story must be completable in ONE OpenRalph iteration (one context window).**

OpenRalph spawns a fresh OpenCode instance per iteration with no memory of previous work. If a story is too big, the LLM runs out of context before finishing and produces broken code.
...
Each criterion must be something OpenRalph can CHECK, not something vague.
...
Frontend stories are NOT complete until visually verified. OpenRalph will use browser tools to navigate to the page, interact with the UI, and confirm changes work.
